...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "input",
        nargs="?",
        default="charts/res-test-suite-ct-{arch}",
        help="Input results file (default: charts/res-test-suite-ct-{arch})",
    )
    parser.add_argument(
        "--metric",
        default="codegen",
    ...
    parser.add_argument(
        "--output",
        default="charts/compiletime_speedup_vs_clang_o1_{arch}.png",
        help="Output plot path (default: charts/compiletime_speedup_vs_clang_o1_{arch}.png)",
    )
    parser.add_argument(
        "--architectures",
        nargs="+",
...
def main() -> None:
    ...
    for arch in args.architectures:
        ...
        if not per_benchmark:
            raise RuntimeError(
                f"No data found for metric '{args.metric}' in {input_path}"
            )
        ...
        plt.axhline(
            1.0, color="black", linestyle="--", linewidth=1, label=args.baseline
        )
        plt.ylabel(f"Compile-time speedup vs {args.baseline} (higher is better)")
        plt.xlabel("Benchmark")
        plt.title(f"{args.metric} speedup per benchmark ({args.baseline} = 1) ({arch})")
