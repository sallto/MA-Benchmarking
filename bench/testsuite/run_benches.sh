...
TPDE_OLD_SIZE=0
for ((i=0; i<3; ++i)); do
	TMP=`lit -v build_llvm_O1/MultiSource/Benchmarks/$name -j 1`
...
for ((i=0; i<3; ++i)); do
	TMP=`lit -v build_llvm_O0/MultiSource/Benchmarks/$name -j 1`
...
for ((i=0; i<3; ++i)); do
	TMP=`lit -v build_tpde_old/MultiSource/Benchmarks/$name -j 1`
