...
for idx in ${!BENCH_NUMBERS[@]}; do
    ...
    NAME_ARR=("clang" "tpde ")
    BUILD_DIRS=("${CLANG_BUILD_DIR}" "${TPDE_BUILD_DIR}")
	LLCS=("${SCRIPT_DIR}/llvm/bin/llc --filetype=obj --relocation-model=pic ${LLC_OPT_MODE}" "${SCRIPT_DIR}/tpde/tpde-llvm/tpde-llc")
    ...
    if [ "${BENCH_NUMBERS[$idx]}" == "648" ]; then
        ...
        python3 "${SCRIPT_DIR}/parse_trace_jsons.py" "${BENCH_NUMBERS[$idx]}" "${CLANG_BUILD_DIR}" "${TPDE_BUILD_DIR}" "true" | tee -a "${SCRIPT_DIR}/${OUT_FILE}"
    else
        python3 "${SCRIPT_DIR}/parse_trace_jsons.py" "${BENCH_NUMBERS[$idx]}" "${CLANG_BUILD_DIR}" "${TPDE_BUILD_DIR}" "${LLC_MODE}" | tee -a "${SCRIPT_DIR}/${OUT_FILE}"
    fi
